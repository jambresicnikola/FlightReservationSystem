...
public class InputData {
    ...
    public static Flight enterFlight(Scanner sc, List<Airline> airlines, List<Pilot> pilots) {
        ...
        Airline airline = airlines.get(selectedIndex - 1);
        ...
        do {
            try {
                ...
                if (selectedIndex < 1 || selectedIndex > pilots.size()) {
                    System.out.println(Messages.PLEASE_CHOOSE_VALID_OPTION);
                } else {
                    break;
                }
        ...
        sc.nextLine();
        Pilot pilot = pilots.get(selectedIndex - 1);
    ...
    private static Integer chooseAirlineIndex(Scanner sc, List<Airline> airlines) throws IllegalAirlineIndexSelectedException {
        Integer selectedIndex = sc.nextInt();

        if (selectedIndex < 1 || selectedIndex > airlines.size()) {
    ...
    public static Booking enterBooking(Scanner sc, List<Passenger> passengers, List<Flight> flights) {
        ...
        do {
            try {
                ...
                if (selectedIndex < 1 || selectedIndex > passengers.size()) {
                    System.out.println(Messages.PLEASE_CHOOSE_VALID_OPTION);
                } else {
                    break;
                }
        ...
        Passenger passenger = passengers.get(selectedIndex - 1);
        ...
        do {
            try {
                ...
                if (selectedIndex < 1 || selectedIndex > flights.size()) {
                    System.out.println(Messages.PLEASE_CHOOSE_VALID_OPTION);
                } else {
                    break;
                }
        ...
        Flight flight = flights.get(selectedIndex - 1);
        ...
        Map<SeatClass, BigDecimal> seatPrices = Map.of(
                SeatClass.ECONOMY, new BigDecimal(100),
                SeatClass.BUSINESS, new BigDecimal(300),
                SeatClass.FIRST_CLASS, new BigDecimal(500)
        );

        SeatPrice seatPrice;
        do {
            try {
                System.out.println("Choose a seat price:");

                Integer i = 1;
                for (SeatClass seatClass : seatPrices.keySet()) {
                    System.out.println("\t" + i + ". " + seatClass + ": " + seatPrices.get(seatClass));
                    i++;
                }
                Integer selectedPrice = sc.nextInt();

                if (selectedPrice < 1 || selectedPrice > seatPrices.size()) {
                    System.out.println("Seat price must be a positive number.");
                    continue;
                }

                SeatClass selectedClass = SeatClass.values()[selectedPrice - 1];
                BigDecimal price = seatPrices.get(selectedClass);

                seatPrice = new SeatPrice(selectedClass, price);
